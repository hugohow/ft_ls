...
/*   Updated: 2019/06/11 02:14:15 by hhow-cho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_time(t_content *content, long flag)
{
	if (flag & FLAG_CAP_U)
	{
		...
		content->time_sec = (unsigned long long)(content->file_stat).st_birthtimespec.tv_sec;	
		content->time_nsec = (unsigned long long)(content->file_stat).st_mtimespec.tv_nsec;	
	}
	else if (flag & FLAG_U)
	{
		content->time = (unsigned long long)(content->file_stat).st_atime;
		content->time_sec = (unsigned long long)(content->file_stat).st_atimespec.tv_sec;
		content->time_nsec = (unsigned long long)(content->file_stat).st_mtimespec.tv_nsec;
	}
	else
	{
		content->time = (unsigned long long)(content->file_stat).st_mtime;
		content->time_sec = (unsigned long long)(content->file_stat).st_mtimespec.tv_sec;
		content->time_nsec = (unsigned long long)(content->file_stat).st_mtimespec.tv_nsec;
	}
}
